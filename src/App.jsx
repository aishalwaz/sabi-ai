...
function MerchantOnboarding({ onBack, community, activeLang = 'en' }) {
  ...
  useEffect(() => {
    ...
    const drawBanner = async () => {
      ...
      if (isPrint) {
        drawCityscape(640, 1200, H - 20, 0.05)
        drawSwoosh(900, 1050, H - 80, 1150, H - 40, W, H - 120, 0.85, 0)
        drawSwoosh(915, 1065, H - 65, 1165, H - 25, W, H - 105, 0.5, 15)

        drawRoundedImage(btcImg, 48, 28, 180, 50)
        drawTagline(48, 86)

        ctx.fillStyle = '#F97316'
        ctx.fillRect(52, 120, 6, 160)

        const leftTextX = 72
        const heroLines = [
          { text: 'BITCOIN', color: '#111111' },
          { text: 'ACCEPTED', color: '#F97316' },
          { text: 'HERE', color: '#111111' },
        ]
        ctx.textAlign = 'left'
        ctx.textBaseline = 'top'
        heroLines.forEach((line, idx) => {
          ...
          ctx.fillText(line.text, leftTextX, 120 + idx * 88)
        })

        ctx.fillStyle = '#F97316'
        ctx.fillRect(72, 390, 48, 3)
        ctx.fillStyle = '#14B8A6'
        ctx.font = '700 18px Arial, sans-serif'
        ctx.fillText('⚡ Lightning payments.', 72, 406)
        ctx.fillStyle = '#374151'
        ctx.font = '400 15px Arial, sans-serif'
        ctx.fillText('Instant. Secure. Borderless.', 72, 430)

        if (loc) {
          drawPinDot(72, 485, 5, '#F97316')
          ctx.fillStyle = '#374151'
          ctx.font = '500 15px Arial, sans-serif'
          ctx.fillText(loc, 84, 472)
        }
        ...
        drawScanHeader(cardX, cardW, cardY + 32, cardY + 18, 36)

        const qrSize = 220
        const qrX = cardX + Math.round((cardW - qrSize) / 2)
        const qrY = cardY + 50
        if (qrImg) ctx.drawImage(qrImg, qrX, qrY, qrSize, qrSize)
        drawBitcoinBadge(cardX + cardW / 2, cardY + 160, 28)

        const phoneX = cardX + cardW / 2 - 7
        drawPhoneIcon(phoneX, cardY + 282, 14, 20, '#374151')
        ctx.fillStyle = '#374151'
        ctx.font = '500 13px Arial, sans-serif'
        ctx.textAlign = 'center'
        ctx.textBaseline = 'top'
        ctx.fillText('No POS needed', cardX + cardW / 2, cardY + 312)
