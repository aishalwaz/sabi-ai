...
function getCommunityFromURL() {
  try {
    ...
    return p.get('community') || 'bitcoin-abuja'
  } catch(e) { return 'bitcoin-abuja' }
}
...
const MERCHANT_STEPS = [
  ...
  { ins:'Step 5 of 6 — Fund your wallet (optional)', q:'Have you added any sats to your wallet? You can skip this — you can receive payments without having sats first.',         yes:'Good.',                               no:'No problem. Move on.', canSkip:true },
...
const MEMBER_STEPS = [
  ...
  { ins:'Step 2 of 4 — Join a Federation',  q:'Open Fedi and tap the Wallet tab. Join any available federation — this creates your Bitcoin wallet. Done?',           yes:'Your wallet is ready.',         no:'Tap the Wallet tab at the bottom of Fedi. You will see options to join a federation. Pick any one.' },
...
FEDI MINI APPS IN ${community.name.toUpperCase()}:
...
LnESIM — buy phone credit and mobile data using Bitcoin Lightning
PayPerQ — AI chat giving access to multiple AI models, pay per prompt
Sats Faucet — claim free 100 sats (community members only)
...
3. Separately join the ${community.name} community by scanning the community QR code
...
5. Transfer from any bank — GTB, Access, Zenith, Opay, Kuda, any bank works
6. Wait 5 to 10 minutes → sats appear in your wallet. No ID required.
...
2. Screenshot the QR code and display it at your counter
3. Customer scans → payment arrives instantly
4. Convert to Naira anytime via Cashwyre Offramp
No POS machine. No fees. Works 24/7.

ABOUT THE 100 SATS FAUCET:
The community has a free Sats Faucet giving new members 100 sats. This is not for spending — 100 sats is about 1 Naira. It is proof of ownership. Your first real Bitcoin. Stack more via Cashwyre — even 500 Naira a week adds up significantly over time. Do not tell people to spend their 100 sats at a merchant. Encourage them to stack more.

WHAT IS BITCOIN: Digital money with a fixed supply of 21 million coins forever. No bank or government controls it. The Naira has lost over 80% of its value since 2020. Bitcoin cannot be inflated.

WHAT IS FEDI: A Bitcoin app combining a Lightning wallet, community chat, and mini apps. Your Bitcoin is held by a federation — a group of trusted community members — not one company.

WALLET BACKUP: Profile → Personal Backup → write recovery words on physical paper. Never screenshot. Those words are your money.

${community.name.toUpperCase()}: A Bitcoin circular economy in ${community.city}. ${community.memberCount} members, ${community.merchantCount} active merchants. Weekly education classes. Led by Aisha Ummi Waziri. Uses Fedi as platform.

KEEP RESPONSES: Match depth to question. Numbers for steps, plain text elsewhere. Warm and human. Always in the user's language. Never use asterisks or markdown.`
...
function drawBanner(canvas, shopName, shopLocation, shopCategory, bannerLang, qrDataURL, community) {
  ...
  ctx.fillStyle = g; ctx.fillRect(0,0,W,isPrint?90:120)
  ...
  const render = (qrImg, fediLogoImg, btcLogoImg) => {
    if (isPrint) {
      const QS=340, QX=W-QS-80, QY=(H-QS)/2-10
      ctx.fillStyle='#F8F8F8'; rrect(ctx,QX-20,QY-20,QS+40,QS+40,20); ctx.fill()
      ctx.strokeStyle='#E8E8E8'; ctx.lineWidth=2; rrect(ctx,QX-20,QY-20,QS+40,QS+40,20); ctx.stroke()
      if (qrImg) ctx.drawImage(qrImg,QX,QY,QS,QS)
      ctx.fillStyle='#8A9BB5'; ctx.font='500 19px Arial'; ctx.textBaseline='top'
      ctx.fillText('Scan to pay with Bitcoin Lightning',QX-20,QY+QS+26)
      ctx.font='40px Arial'; ctx.fillText(catEmoji,80,112)
      ctx.fillStyle='#1B2232'; ctx.font='bold 50px Arial'
      ctx.fillText(`${name}`,80,168)
      ctx.fillStyle='#1B2232'; ctx.font='bold 42px Arial'
      ctx.fillText('now accepts',80,232)
      ctx.fillStyle='#F97316'; ctx.font='bold 72px Arial'
      ctx.fillText('Bitcoin  ₿',80,282)
      if (loc) { ctx.fillStyle='#8A9BB5'; ctx.font='400 24px Arial'; ctx.fillText(`📍 ${loc}`,80,370) }
      ctx.fillStyle='#4A5A72'; ctx.font='500 26px Arial'
      ctx.fillText(s1,80,loc?408:370)
      if (s2) { ctx.fillStyle='#8A9BB5'; ctx.font='400 22px Arial'; ctx.fillText(s2,80,loc?444:406) }
      ctx.fillStyle='#2DD4BF'; ctx.font='500 18px Arial'
      ctx.fillText('⚡ Instant  ·  ✓ No POS machine  ·  ✓ No transfer fees  ·  ✓ Works 24/7',80,480)
      ctx.fillStyle='#FFFFFF'; ctx.font='500 19px Arial'; ctx.textBaseline='middle'
      ctx.fillText('Bitcoin Abuja  ·  sabibtc.vercel.app',40,H-35)
      if (btcLogoImg) { ctx.drawImage(btcLogoImg, W-256, (90-72)/2, 240, 72) }
      if (fediLogoImg) { ctx.drawImage(fediLogoImg, W-120, H-52, 100, 32) }
    } else {
      const QS=300, QX=(W-QS)/2, QY=220
      ctx.fillStyle='#F8F8F8'; rrect(ctx,QX-24,QY-24,QS+48,QS+48,24); ctx.fill()
      if (qrImg) ctx.drawImage(qrImg,QX,QY,QS,QS)
      ctx.fillStyle='#8A9BB5'; ctx.font='500 22px Arial'; ctx.textAlign='center'; ctx.textBaseline='top'
      ctx.fillText('Scan to pay with Bitcoin',W/2,QY+QS+22)
      ctx.fillStyle='#FFFFFF'; ctx.font='bold 34px Arial'; ctx.textBaseline='middle'
      ctx.fillText(`${catEmoji} ${name}`,W/2,50)
      if (loc) { ctx.fillStyle='rgba(255,255,255,0.6)'; ctx.font='400 20px Arial'; ctx.fillText(`📍 ${loc}`,W/2,78) }
      ctx.fillStyle='#1B2232'; ctx.font='bold 52px Arial'; ctx.textBaseline='top'
      ctx.fillText('now accepts',W/2,600)
      ctx.fillStyle='#F97316'; ctx.font='bold 72px Arial'
      ctx.fillText('Bitcoin  ₿',W/2,660)
      ctx.fillStyle='#4A5A72'; ctx.font='500 28px Arial'; ctx.fillText(s1,W/2,758)
      if (s2) { ctx.fillStyle='#8A9BB5'; ctx.font='400 24px Arial'; ctx.fillText(s2,W/2,796) }
      ctx.fillStyle='#FFFFFF'; ctx.font='500 20px Arial'; ctx.textBaseline='middle'
      ctx.fillText('Bitcoin Abuja  ·  Powered by Fedi',W/2,H-60)
      ctx.textAlign='left'; ctx.textBaseline='top'
      if (btcLogoImg) { ctx.drawImage(btcLogoImg,(W-180)/2,118,180,46) }
      if (fediLogoImg) { ctx.drawImage(fediLogoImg,W-110,H-48,90,28) }
    }
  }
  const fediImg = new Image(); const btcImg = new Image()
  let loaded = 0
  const tryRender = () => { loaded++; if(loaded>=2) { if(qrDataURL){const qi=new Image();qi.onload=()=>render(qi,fediImg,btcImg);qi.src=qrDataURL}else render(null,fediImg,btcImg) } }
  fediImg.onload = tryRender; fediImg.onerror = tryRender
  btcImg.onload  = tryRender; btcImg.onerror  = tryRender
...
.dl-btn{flex:1;padding:13px;border-radius:14px;border:none;font-weight:700;font-size:14px;cursor:pointer;font-family:inherit;transition:transform 0.15s;display:flex;align-items:center;justify-content:center;gap:6px;}
.dl-btn:hover{transform:scale(1.03);}
.cat-btn{padding:11px 12px;border-radius:12px;border:1px solid rgba(212,168,67,.2);background:rgba(212,168,67,.05);cursor:pointer;font-family:inherit;text-align:left;transition:all 0.15s;-webkit-tap-highlight-color:transparent;}
.cat-btn:hover{border-color:rgba(212,168,67,.4);background:rgba(212,168,67,.1);}
.cat-btn.selected{border-color:#D4A843;background:rgba(212,168,67,.15);}
`
...
const Icons = {
  ...
  Check: ({color='#2DD4BF'}) => <svg width="20" height="20" viewBox="0 0 24 24" fill="none" style={{flexShrink:0,animation:'checkPop 0.4s cubic-bezier(0.22,1,0.36,1) both'}}><circle cx="12" cy="12" r="10" fill={color==='#2DD4BF'?'rgba(45,212,191,0.15)':color==='#F97316'?'rgba(249,115,22,0.12)':'rgba(167,139,250,0.12)'} stroke={color} strokeWidth="1.5"/><path d="M8 12l3 3 5-5" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>,
...
function ShareCard({ type, shopName, shopCategory, onClose }) {
  ...
  useEffect(() => {
    ...
    if (isMerchant) {
      ...
      bg.addColorStop(0,'#1B2232'); bg.addColorStop(1,'#2A3650')
      ctx.fillStyle=bg; ctx.fillRect(0,0,W,H)
      ctx.strokeStyle='#D4A843'; ctx.lineWidth=8; ctx.strokeRect(4,4,W-8,H-8)
      ctx.fillStyle='#D4A843'; ctx.font='bold 100px Arial'; ctx.textAlign='center'; ctx.textBaseline='top'
      ctx.fillText(cat?.emoji||'🏪',W/2,80)
      ctx.fillText('₿',W/2,200)
      ctx.fillStyle='#EDF2FF'; ctx.font='bold 58px Arial'; ctx.fillText(shopName||'Your Shop',W/2,330)
      ctx.font='bold 52px Arial'; ctx.fillText('now accepts Bitcoin',W/2,410)
      ctx.fillStyle='#D4A843'; ctx.fillRect(200,490,W-400,3)
      ctx.fillStyle='#8A9BB5'; ctx.font='500 36px Arial'; ctx.fillText('Bitcoin Abuja',W/2,520)
      ctx.fillStyle='#4A5A72'; ctx.font='400 30px Arial'; ctx.fillText('Scan to pay — instant, no POS needed',W/2,580)
      ctx.fillStyle='#2DD4BF'; ctx.font='500 26px Arial'; ctx.fillText('Powered by Fedi  ·  sabibtc.vercel.app',W/2,640)
      ctx.fillStyle='#D4A843'; ctx.font='600 22px Arial'; ctx.fillText('Official Bitcoin Merchant · Bitcoin Abuja',W/2,H-70)
    } else {
      const bg=ctx.createLinearGradient(0,0,W,H)
      bg.addColorStop(0,'#1B1A2E'); bg.addColorStop(1,'#2A2040')
      ctx.fillStyle=bg; ctx.fillRect(0,0,W,H)
      ctx.strokeStyle='#F97316'; ctx.lineWidth=8; ctx.strokeRect(4,4,W-8,H-8)
      ctx.fillStyle='#F97316'; ctx.font='bold 120px Arial'; ctx.textAlign='center'; ctx.textBaseline='top'
      ctx.fillText('₿',W/2,80)
      ctx.fillStyle='#EDF2FF'; ctx.font='bold 58px Arial'; ctx.fillText('I just joined Bitcoin Abuja.',W/2,250)
      ctx.fillStyle='#F97316'; ctx.font='bold 56px Arial'; ctx.fillText('I own Bitcoin.',W/2,330)
      ctx.fillStyle='#EDF2FF'; ctx.font='bold 52px Arial'; ctx.fillText('No bank controls my money.',W/2,410)
      ctx.fillStyle='#F97316'; ctx.fillRect(200,490,W-400,3)
      ctx.fillStyle='#8A9BB5'; ctx.font='500 34px Arial'
      ctx.fillText('Building financial freedom',W/2,520)
      ctx.fillText('— one sat at a time.',W/2,566)
      ctx.fillStyle='#A78BFA'; ctx.font='500 28px Arial'; ctx.fillText('Bitcoin Abuja · Powered by Fedi',W/2,640)
      ctx.fillStyle='#F97316'; ctx.font='600 24px Arial'; ctx.fillText('sabibtc.vercel.app',W/2,H-70)
    }
  }, [isMerchant, shopName, shopCategory])

  const download = () => {
    const a=document.createElement('a')
    a.download=isMerchant?`${(shopName||'merchant').toLowerCase().replace(/\s+/g,'-')}-bitcoin-card.png`:'bitcoin-abuja-member-card.png'
  ...
  return (
    <div className="done-pop" style={{padding:16,display:'flex',flexDirection:'column',gap:14}}>
      <div style={{fontSize:11,color:isMerchant?B.teal:B.orange,fontWeight:600,letterSpacing:1,textTransform:'uppercase',textAlign:'center'}}>
        {isMerchant?'✦ Your Bitcoin Merchant Card':'✦ Your Bitcoin Abuja Card'}
      </div>
      <div style={{borderRadius:14,overflow:'hidden',boxShadow:'0 8px 32px rgba(0,0,0,.5)'}}>
        <canvas ref={canvasRef} style={{width:'100%',height:'auto',display:'block'}}/>
      </div>
      <div style={{fontSize:12,color:B.mid,textAlign:'center',lineHeight:1.6}}>Download and share on X, WhatsApp, or anywhere.</div>
      <div style={{display:'flex',gap:10}}>
        <button className="dl-btn" style={{background:`linear-gradient(135deg,${B.gold},${B.goldD})`,color:'#0D0A00',boxShadow:'0 3px 16px rgba(212,168,67,.4)'}} onClick={download}>
...
function CommunityRequestForm({ onClose }) {
  const [form, setForm] = useState({ name:'', community:'', city:'', country:'', fediLink:'', email:'' })
  ...
  if (submitted) {
    return (
      <div className="done-pop" style={{padding:20,display:'flex',flexDirection:'column',alignItems:'center',gap:16,textAlign:'center'}}>
        <div style={{fontSize:44}}>🎉</div>
        <div style={{fontSize:18,fontWeight:700,color:B.teal}}>Request Received!</div>
        <div style={{fontSize:13,color:B.mid,lineHeight:1.65}}>Thank you for your interest in bringing Sabi AI to your community. Aisha will be in touch with you directly to set up your community onboarding.</div>
        <div style={{fontSize:12,color:B.dim}}>Built by Bitcoin Abuja · sabibtc.vercel.app</div>
        <button className="action-cta" onClick={onClose} style={{padding:'12px 28px',borderRadius:14,border:'none',background:`linear-gradient(135deg,${B.gold},${B.goldD})`,color:'#0D0A00',fontWeight:700,fontSize:14,cursor:'pointer',fontFamily:'inherit'}}>Back to Sabi AI</button>
      </div>
    )
  }
  ...
  const inp = (label, key, placeholder) => (
    <div style={{display:'flex',flexDirection:'column',gap:6}}>
      ...
      <input value={form[key]} onChange={e=>setForm(p=>({...p,[key]:e.target.value}))} placeholder={placeholder}
        style={{padding:'12px 14px',background:B.navy,border:`1px solid ${B.navyB}`,borderRadius:12,fontSize:14,color:B.white,fontFamily:'inherit',outline:'none'}}/>
    </div>
  )
  ...
  return (
    <div style={{padding:16,display:'flex',flexDirection:'column',gap:14}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <div style={{fontSize:11,color:B.teal,fontWeight:600,letterSpacing:1,textTransform:'uppercase'}}>Bring Sabi to Your Community</div>
        <button onClick={onClose} style={{fontSize:12,color:B.dim,background:'transparent',border:'none',cursor:'pointer',fontFamily:'inherit'}}>Exit</button>
      </div>
      <div style={{background:B.navyL,border:`1px solid ${B.tealB}`,borderRadius:16,padding:14,fontSize:13,color:B.mid,lineHeight:1.6}}>
        Sabi AI can be customised for any Bitcoin community on Fedi. Fill in your details and we will set up your community onboarding experience — same tool, your branding and community link.
      </div>
      ...
      <button onClick={handleSubmit} disabled={!(form.name&&form.community&&form.city)||loading}
        style={{width:'100%',padding:14,borderRadius:14,border:'none',background:(form.name&&form.community&&form.city)?`linear-gradient(135deg,${B.teal},#0ea5a0)`:B.navyLL,color:(form.name&&form.community&&form.city)?'#0D1A1A':B.dim,fontWeight:700,fontSize:15,cursor:(form.name&&form.community&&form.city)?'pointer':'default',fontFamily:'inherit',boxShadow:(form.name&&form.community&&form.city)?'0 3px 16px rgba(45,212,191,.3)':'none'}}>
...
function StepFlow({ title, titleColor, steps, currentStep, pct, dotDone, dotActive, feedback, onYes, onNo, onSkip, onExit, footer }) {
  const step = steps[currentStep]
  return (
    <div className="flow-in" style={{padding:16,display:'flex',flexDirection:'column',gap:12}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <span style={{fontSize:11,color:titleColor,fontWeight:600,letterSpacing:1,textTransform:'uppercase'}}>{title}</span>
      ...
      <div style={{height:4,background:'rgba(255,255,255,0.06)',borderRadius:4,overflow:'hidden'}}>
        <div style={{height:'100%',width:`${pct}%`,background:`linear-gradient(90deg,${titleColor},${B.gold})`,borderRadius:4,transition:'width 0.5s cubic-bezier(0.22,1,0.36,1)'}}/>
      </div>
      <div style={{display:'flex',gap:8,justifyContent:'center'}}>
        {steps.map((_,i)=>(
          <div key={i} style={{width:10,height:10,borderRadius:'50%',background:i<currentStep?dotDone:i===currentStep?dotActive:'rgba(255,255,255,0.08)',boxShadow:i<currentStep?`0 0 6px ${dotDone}`:i===currentStep?`0 0 8px ${dotActive}`:'none',transition:'background 0.3s,box-shadow 0.3s'}}/>
        ))}
      </div>
      <div style={{background:B.navyL,borderRadius:20,padding:20,border:`1px solid ${feedback==='yes'?`${titleColor}40`:feedback==='no'?B.redB:B.navyB}`,transition:'border-color 0.3s'}}>
        <div style={{fontSize:10,color:B.dim,letterSpacing:1,textTransform:'uppercase',marginBottom:10}}>{step.ins}</div>
        <div style={{fontSize:15,fontWeight:600,color:B.white,lineHeight:1.5,marginBottom:18}}>{step.q}</div>
        {feedback==='yes'&&(
        ...
        {feedback==='no'&&(
          <div style={{padding:'12px 13px',background:B.redF,border:`1px solid ${B.redB}`,borderRadius:12,marginBottom:14}}>
            <div style={{fontSize:13,color:B.mid,lineHeight:1.6,marginBottom:10}}>{step.no}</div>
            <button onClick={()=>onNo('reset')} style={{padding:'8px 18px',borderRadius:20,border:`1px solid ${B.navyB}`,background:'transparent',color:B.white,fontSize:12,cursor:'pointer',fontFamily:'inherit'}}>I'm ready now</button>
          </div>
        )}
        {!feedback&&(
          <div style={{display:'flex',gap:10,flexDirection:'column'}}>
            <div style={{display:'flex',gap:10}}>
              <button className="yes-btn" onClick={onYes} style={{flex:1,padding:13,borderRadius:14,border:'none',background:`linear-gradient(135deg,${titleColor},${titleColor}cc)`,color:titleColor===B.teal?'#0D1A1A':'white',fontWeight:700,fontSize:14,cursor:'pointer',fontFamily:'inherit',boxShadow:`0 3px 16px ${titleColor}50`}}>Yes ✓</button>
              <button className="no-btn" onClick={()=>onNo('no')} style={{flex:1,padding:13,borderRadius:14,border:`1px solid ${B.redB}`,background:B.redF,color:B.red,fontWeight:600,fontSize:14,cursor:'pointer',fontFamily:'inherit'}}>Not yet</button>
            </div>
            {step.canSkip&&<button onClick={onSkip} style={{background:'transparent',border:'none',color:B.dim,fontSize:12,cursor:'pointer',fontFamily:'inherit',textAlign:'center'}}>Skip this step</button>}
          </div>
        )}
      </div>
      {footer&&<div style={{fontSize:11,color:B.dim,textAlign:'center'}}>{footer}</div>}
    </div>
  )
}

function JoinCommunityScreen({ community, onDone, onExit, titleColor, title }) {
  return (
    <div className="flow-in" style={{padding:16,display:'flex',flexDirection:'column',gap:12}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <span style={{fontSize:11,color:titleColor,fontWeight:600,letterSpacing:1,textTransform:'uppercase'}}>{title}</span>
      ...
      <div style={{background:B.navyL,borderRadius:20,padding:20,border:`1px solid ${B.orangeB}`,display:'flex',flexDirection:'column',alignItems:'center',gap:14}}>
        ...
        <div style={{fontSize:12,color:B.gold,background:B.goldF,border:`1px solid ${B.goldB}`,borderRadius:10,padding:'10px 12px',lineHeight:1.55,textAlign:'center',width:'100%'}}>
          Join a federation from the Wallet tab first — that creates your Bitcoin wallet. Then join this community.
        </div>
        <div style={{fontSize:13,color:B.mid,textAlign:'center',lineHeight:1.6}}>Scan this QR code inside Fedi or tap the button below.</div>
        <div style={{background:'white',borderRadius:14,padding:12}}>
          <img src={community.communityQR} alt={`${community.name} QR`} style={{width:140,height:140,display:'block'}}/>
        </div>
        <div style={{fontSize:11,color:B.dim,textAlign:'center'}}>Open Fedi → tap the scan icon → scan this QR code</div>
...
function MerchantOnboarding({ onExit, community }) {
  ...
  useEffect(() => {
    if (phase==='banner'&&canvasRef.current) {
      ...
      drawBanner(canvasRef.current, shopName, shopLocation, shopCategory, bannerLang, qrData, community)
    }
  }, [phase, bannerMode, qrData, shopName, shopLocation, shopCategory, bannerLang, community])
  ...
  const handleYes = () => {
    setFeedback('yes')
    setTimeout(()=>{ setFeedback(null); setStep(p=>p+1) }, 1200)
  }
  ...
  if (phase==='sharecard') return <ShareCard type="merchant" shopName={shopName} shopCategory={shopCategory} onClose={onExit}/>
    ...
    return (
      <div style={{padding:16,display:'flex',flexDirection:'column',gap:14}}>
        ...
        <div style={{display:'flex',gap:8}}>
          {['print','square'].map(m=>(
            <button key={m} className={`banner-tab${bannerMode===m?' active':''}`}
              onClick={()=>{ setBannerMode(m); if(canvasRef.current){canvasRef.current.dataset.mode=m; drawBanner(canvasRef.current,shopName,shopLocation,shopCategory,bannerLang,qrData,community)} }}>
        ...
        <div style={{display:'flex',gap:10}}>
          <button className="dl-btn" style={{background:`linear-gradient(135deg,${B.gold},${B.goldD})`,color:'#0D0A00',boxShadow:'0 3px 16px rgba(212,168,67,.4)'}}
            onClick={()=>{ const n=(shopName||'merchant').toLowerCase().replace(/[^a-z0-9]/g,'-'); const a=document.createElement('a'); a.download=`${n}-banner-${bannerMode}.png`; a.href=canvasRef.current.toDataURL('image/png'); a.click() }}>
        ...
        <button onClick={onExit} style={{width:'100%',padding:12,borderRadius:14,border:`1px solid ${B.navyB}`,background:'transparent',color:B.mid,fontSize:13,cursor:'pointer',fontFamily:'inherit'}}>← Back to Sabi AI</button>
      </div>
    )
  }
        ...
        <div style={{background:B.navyL,borderRadius:20,padding:20,border:`1px solid ${B.tealB}`,display:'flex',flexDirection:'column',gap:14}}>
          ...
          {!qrData?(
            ...
            <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:10}}>
              ...
              <button onClick={()=>document.getElementById('merQR').click()} style={{background:'transparent',border:`1px solid ${B.navyB}`,color:B.dim,fontSize:12,cursor:'pointer',fontFamily:'inherit',padding:'7px 16px',borderRadius:20}}>Upload different image</button>
            </div>
          )}
          <input type="file" id="merQR" accept="image/*" onChange={e=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=ev=>setQrData(ev.target.result);r.readAsDataURL(f);e.target.value=''}} style={{display:'none'}}/>
      ...
      <div style={{margin:16,background:B.navyL,border:`1px solid ${B.goldB}`,borderRadius:20,padding:22,display:'flex',flexDirection:'column',gap:10}}>
        ...
        {[
          ...
          <button key={o.key} onClick={()=>{setBannerLang(o.key);setPhase('upload')}}
            style={{padding:'13px 16px',borderRadius:14,border:`1px solid ${B.navyB}`,background:B.navy,cursor:'pointer',fontFamily:'inherit',textAlign:'left',width:'100%'}}>
  ...
  if (phase==='location') {
    return (
      <div style={{margin:16,background:B.navyL,border:`1px solid ${B.goldB}`,borderRadius:20,padding:22,display:'flex',flexDirection:'column',gap:14}}>
        <div style={{fontSize:15,fontWeight:600,color:B.white,textAlign:'center',lineHeight:1.5}}>Where is your shop located?</div>
        <div style={{fontSize:13,color:B.mid,textAlign:'center'}}>This helps customers find you on BTCMap.</div>
        ...
        <button onClick={()=>{setShopLocation('');setPhase('lang')}} style={{background:'transparent',border:'none',color:B.dim,fontSize:12,cursor:'pointer',fontFamily:'inherit',textAlign:'center'}}>
  ...
  if (phase==='category') {
    return (
      <div style={{margin:16,background:B.navyL,border:`1px solid ${B.goldB}`,borderRadius:20,padding:22,display:'flex',flexDirection:'column',gap:14}}>
        <div style={{fontSize:15,fontWeight:600,color:B.white,textAlign:'center'}}>What type of business is this?</div>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
          {BUSINESS_CATEGORIES.map(c=>(
            <button key={c.id} className={`cat-btn${shopCategory===c.id?' selected':''}`} onClick={()=>setShopCategory(c.id)}>
              ...
              <div style={{fontSize:12,fontWeight:500,color:B.white,lineHeight:1.3}}>{c.label}</div>
            </button>
          ))}
        </div>
        <button onClick={()=>setPhase('location')} disabled={!shopCategory}
          style={{padding:14,borderRadius:14,border:'none',background:shopCategory?`linear-gradient(135deg,${B.teal},#0ea5a0)`:B.navyLL,color:shopCategory?'#0D1A1A':B.dim,fontWeight:600,fontSize:15,cursor:shopCategory?'pointer':'default',fontFamily:'inherit'}}>
          Continue
        </button>
      </div>
    )
  ...
  if (phase==='name') {
    return (
      <div style={{margin:16,background:B.navyL,border:`1px solid ${B.goldB}`,borderRadius:20,padding:22,display:'flex',flexDirection:'column',gap:14}}>
        ...
        <button onClick={()=>{setShopName('');setPhase('category')}} style={{background:'transparent',border:'none',color:B.dim,fontSize:12,cursor:'pointer',fontFamily:'inherit',textAlign:'center'}}>
  ...
  if (phase==='join') {
    return <JoinCommunityScreen community={community} onDone={()=>{setPhase('steps');setStep(p=>p+1)}} onExit={onExit} titleColor={B.teal} title="Merchant Setup"/>
  }

  return (
    <StepFlow title="Merchant Setup" titleColor={B.teal} steps={MERCHANT_STEPS} currentStep={step} pct={pct}
      dotDone={B.teal} dotActive={B.gold} feedback={feedback}
      onYes={handleYes} onNo={handleNo} onSkip={()=>setStep(p=>p+1)} onExit={onExit}
      footer="Bitcoin Abuja · Powered by Fedi"/>
  )
}

function MemberOnboarding({ onExit, community }) {
  ...
  if (phase==='sharecard') return <ShareCard type="member" onClose={onExit}/>
  ...
  if (phase==='done') {
    return (
      <div className="done-pop" style={{margin:16,background:B.navyL,border:`1px solid ${B.orangeB}`,borderRadius:20,padding:22,display:'flex',flexDirection:'column',alignItems:'center',gap:14}}>
        <div style={{fontSize:44}}>₿</div>
        <div style={{fontSize:18,fontWeight:700,color:B.orange,textAlign:'center',lineHeight:1.4}}>Welcome to {community.name}!</div>
        <div style={{fontSize:13,color:B.mid,textAlign:'center',lineHeight:1.65}}>You are now part of a real Bitcoin circular economy in Nigeria. Your sats are yours — no bank, no middleman.</div>
        <div style={{width:'100%',background:B.navy,border:'1px solid rgba(212,168,67,.2)',borderRadius:14,padding:16,display:'flex',flexDirection:'column',gap:8}}>
          ...
          <div style={{fontSize:13.5,color:B.white,lineHeight:1.55}}>The {community.name} Sats Faucet has 100 free sats for you. This is your first real Bitcoin — proof of ownership. Stack more via Cashwyre when you are ready. Even 500 Naira a week adds up significantly over time.</div>
  ...
  if (phase==='join') {
    return <JoinCommunityScreen community={community} onDone={()=>{setPhase('steps');setStep(p=>p+1)}} onExit={onExit} titleColor={B.orange} title="New Member Setup"/>
  }
        ...
        <div className="card-slide" style={{background:B.navyL,border:`1px solid ${B.purpleB}`,borderRadius:20,padding:20,display:'flex',flexDirection:'column',gap:14}}>
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            ...
            {isLast?(
              ...
              <button className="learn-next" onClick={()=>setLearnStep(p=>p+1)}
  ...
  if (phase==='choice') {
    return (
      <div className="flow-in" style={{padding:16,display:'flex',flexDirection:'column',gap:12}}>
        ...
        <div style={{display:'flex',gap:8,justifyContent:'center'}}>
          {[0,1,2,3].map(i=><div key={i} style={{width:10,height:10,borderRadius:'50%',background:i===0?B.gold:'rgba(255,255,255,0.08)',boxShadow:i===0?`0 0 8px ${B.gold}`:'none'}}/>)}
        </div>
        <div style={{background:B.navyL,borderRadius:20,padding:20,border:`1px solid ${B.navyB}`}}>
          ...
          <div style={{fontSize:15,fontWeight:600,color:B.white,lineHeight:1.5,marginBottom:18}}>What brings you to {community.name}?</div>
          {[
            ...
            <button key={i} className="choice-btn"
              onClick={()=>{if(c.next==='learn'){setPhase('learn')}else{setPhase('steps');setStep(0)}}}
              style={{width:'100%',padding:'12px 14px',borderRadius:12,border:`1px solid ${B.orangeB}`,background:B.orangeF,color:B.white,fontSize:13,cursor:'pointer',fontFamily:'inherit',textAlign:'left',marginBottom:i<2?8:0}}>
              {c.label}
            </button>
          ))}
        </div>
  ...
  return (
    <StepFlow title="New Member Setup" titleColor={B.orange} steps={MEMBER_STEPS} currentStep={step} pct={pct}
      dotDone={B.orange} dotActive={B.gold} feedback={feedback}
      onYes={handleYes} onNo={handleNo} onSkip={()=>setStep(p=>p+1)} onExit={onExit}
      footer="Bitcoin Abuja · Powered by Fedi"/>
  )
}
export default function App() {
  ...
  const startRecording = useCallback(()=>{
    ...
    const go=()=>{
      ...
      r.continuous=false; r.interimResults=!isAndroid
      r.onstart=()=>{setIsRecording(true);setMicError('')}
    ...
    <>
      ...
      <div className={splashDone?'chat-in':''} style={{background:B.navy,minHeight:'100dvh',maxWidth:440,margin:'0 auto',fontFamily:"'Satoshi',-apple-system,sans-serif",color:B.white,display:'flex',flexDirection:'column'}}>
        ...
        <div style={{padding:'12px 16px',background:B.navyL,borderBottom:`1px solid ${B.navyB}`,display:'flex',justifyContent:'space-between',alignItems:'center',position:'sticky',top:0,zIndex:10}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            ...
            <a href="/stats" target="_blank" rel="noopener noreferrer"
              style={{fontSize:10,color:B.dim,textDecoration:'none',letterSpacing:0.5,padding:'4px 8px',borderRadius:8,border:`1px solid ${B.dim}`}}>
              Stats ↗
            </a>
        ...
        <div style={{flex:1,overflowY:'auto',display:'flex',flexDirection:'column'}}>
          {mode==='merchant'&&<MerchantOnboarding onExit={()=>setMode('chat')} community={community}/>}
          {mode==='member'  &&<MemberOnboarding   onExit={()=>setMode('chat')} community={community}/>}
          {mode==='request' &&<CommunityRequestForm onClose={()=>setMode('chat')}/>}
          ...
          {mode==='chat'&&!hasMessages&&!isLoading&&(
            <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'32px 22px 60px'}}>
              ...
              <p className="w2" style={{fontSize:11,color:B.dim,textAlign:'center',marginBottom:24}}>Joined by {community.memberCount} members across Nigeria</p>
              ...
              {voiceSupported&&(
                <div className="w4" style={{marginTop:18,display:'flex',alignItems:'center',gap:6,color:B.dim,fontSize:11}}>
