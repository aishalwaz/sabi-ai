  ...
  useEffect(() => {
    ...
    const drawBanner = async () => {
      ...
      if (isPrint) {
        ...
        drawLogo(btcImg, 50, 30, 180, 50)
        if (fediImg) ctx.drawImage(fediImg, W - 180, 34, 130, 36)
        ctx.textAlign = 'left'
        ctx.textBaseline = 'top'
        ctx.fillStyle = '#F97316'
        ctx.font = '800 24px Satoshi, Arial'
        ctx.fillText(bl.title, 50, 88)
        if (bl.localTitle) {
          ctx.fillStyle = '#D4A843'
          ctx.font = '700 18px Satoshi, Arial'
          ctx.fillText(bl.localTitle, 50, 118)
        }
        ctx.fillStyle = '#0B1220'
        ctx.font = '900 56px Satoshi, Arial'
        const nameY = fitText(ctx, name, 50, bl.localTitle ? 152 : 136, 600, 62, 2)
        ctx.fillStyle = '#2DD4BF'
        ctx.font = '700 20px Satoshi, Arial'
        ctx.fillText(bl.lightning, 50, nameY + 64)
        if (bl.localLightning) {
          ctx.fillStyle = '#2DD4BF'
          ctx.font = '500 16px Satoshi, Arial'
          ctx.fillText(bl.localLightning, 50, nameY + 90)
        }
        if (loc) {
          ctx.fillStyle = '#8A9BB5'
          ctx.font = '500 18px Satoshi, Arial'
          ctx.fillText('📍 ' + loc, 50, bl.localLightning ? nameY + 116 : nameY + 96)
        }
        const qrSize = 220
        const qrX = W - qrSize - 80
        const qrY = H / 2 - qrSize / 2 - 20
        ctx.fillStyle = '#F8F9FA'
        ctx.strokeStyle = '#E5E7EB'
        ctx.lineWidth = 1.5
        rrect(ctx, qrX - 20, qrY - 20, qrSize + 40, qrSize + 82, 14)
        ctx.fill()
        ctx.stroke()
        if (qrImg) ctx.drawImage(qrImg, qrX, qrY, qrSize, qrSize)
        ctx.fillStyle = '#0B1220'
        ctx.font = '700 15px Satoshi, Arial'
        ctx.textAlign = 'center'
        ctx.fillText('Scan to pay', qrX + qrSize / 2, qrY + qrSize + 16)
        ctx.fillStyle = '#2DD4BF'
        ctx.font = '600 13px Satoshi, Arial'
        ctx.fillText('No POS needed', qrX + qrSize / 2, qrY + qrSize + 34)
        ctx.fillStyle = '#8A9BB5'
        ctx.font = '500 12px Satoshi, Arial'
        ctx.fillText('Bitcoin Abuja · Powered by Fedi', qrX + qrSize / 2, qrY + qrSize + 54)
      } else {
        ctx.fillStyle = '#FFFFFF'
        ctx.fillRect(0, 0, W, H)
        ...
        ctx.textBaseline = 'top'
        ctx.fillStyle = '#F97316'
        ctx.font = '800 40px Satoshi, Arial'
        ctx.fillText(bl.title, W / 2, 120)
        if (bl.localTitle) {
          ctx.fillStyle = '#D4A843'
          ctx.font = '700 28px Satoshi, Arial'
          ctx.fillText(bl.localTitle, W / 2, 168)
        }
        ctx.fillStyle = '#0B1220'
        ctx.font = '900 72px Satoshi, Arial'
        fitText(ctx, name, W / 2, bl.localTitle ? 210 : 188, W - 120, 80, 2)
        ctx.fillStyle = '#2DD4BF'
        ctx.font = '700 30px Satoshi, Arial'
        ctx.fillText(bl.lightning, W / 2, 388)
        if (bl.localLightning) {
          ctx.fillStyle = '#2DD4BF'
          ctx.font = '500 22px Satoshi, Arial'
          ctx.fillText(bl.localLightning, W / 2, 426)
        }
        if (loc) {
          ctx.fillStyle = '#8A9BB5'
          ctx.font = '500 28px Satoshi, Arial'
          ctx.fillText('📍 ' + loc, W / 2, 450)
        }
        const sqQrSize = 340
        const sqQrX = W / 2 - sqQrSize / 2
        const sqQrY = loc ? 508 : 468
        ctx.fillStyle = '#FFFFFF'
        ctx.strokeStyle = '#E5E7EB'
        ctx.lineWidth = 2
        rrect(ctx, sqQrX - 16, sqQrY - 16, sqQrSize + 32, sqQrSize + 32, 16)
        ctx.fill()
        ctx.stroke()
        if (qrImg) ctx.drawImage(qrImg, sqQrX, sqQrY, sqQrSize, sqQrSize)
        ctx.fillStyle = '#0B1220'
        ctx.font = '700 24px Satoshi, Arial'
        ctx.fillText('Scan to pay · No POS needed', W / 2, sqQrY + sqQrSize + 24)
        ctx.fillStyle = '#8A9BB5'
