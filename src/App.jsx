...
function MerchantOnboarding({ onBack, community, activeLang = 'en' }) {
  ...
  useEffect(() => {
    ...
    const drawBanner = async () => {
      ...
      if (isPrint) {
        // Bright white background for physical print
        ...
        // Bitcoin Abuja logo top left
        drawLogo(btcImg, 60, 40, 200, 56)

        // Fedi logo top right — bigger and properly sized
        if (fediImg) {
          ctx.drawImage(fediImg, W - 200, 48, 140, 40)
        }
        ...
        ctx.fillText('BITCOIN ACCEPTED HERE', W / 2, 120)

        // Shop name
        ctx.fillStyle = '#0B1220'
        ctx.font = '900 80px Satoshi, Arial'
        fitText(ctx, name, W / 2, 178, W - 120, 88, 2)

        // Lightning line
        ctx.fillStyle = '#2DD4BF'
        ctx.font = '700 32px Satoshi, Arial'
        ctx.fillText('⚡ Lightning payments. Instant settlement.', W / 2, 360)

        // Location if present
        if (loc) {
          ctx.fillStyle = '#8A9BB5'
          ctx.font = '500 26px Satoshi, Arial'
          ctx.fillText('📍 ' + loc, W / 2, 408)
        }

        // QR code centered
        const qrY = loc ? 460 : 420
        ctx.fillStyle = '#FFFFFF'
        rrect(ctx, W / 2 - 160, qrY, 320, 320, 20)
        ctx.fill()
        ctx.strokeStyle = '#E5E7EB'
        ctx.lineWidth = 2
        rrect(ctx, W / 2 - 160, qrY, 320, 320, 20)
        ctx.stroke()
        if (qrImg) ctx.drawImage(qrImg, W / 2 - 140, qrY + 20, 280, 280)

        ctx.fillStyle = '#0B1220'
        ctx.font = '700 22px Satoshi, Arial'
        ctx.fillText('Scan to pay · No POS needed', W / 2, qrY + 330)

        // Footer
        ctx.fillStyle = '#8A9BB5'
        ctx.font = '500 18px Satoshi, Arial'
        ctx.fillText('Bitcoin Abuja · Powered by Fedi', W / 2, H - 40)
      } else {
        ctx.fillStyle = 'rgba(255,255,255,0.04)'
