  ...
  useEffect(() => {
    ...
    const drawBanner = async () => {
      ...
      if (isPrint) {
        ...
        ctx.font = '800 28px Satoshi, Arial'
        ctx.fillText('BITCOIN ACCEPTED HERE', 50, 100)
        ctx.fillStyle = '#0B1220'
        ctx.font = '900 64px Satoshi, Arial'
        const nameY = fitText(ctx, name, 50, 148, 620, 72, 2)
        ctx.fillStyle = '#2DD4BF'
        ctx.font = '700 26px Satoshi, Arial'
        ctx.fillText('⚡ Lightning payments. Instant settlement.', 50, nameY + 80)
        if (loc) {
          ctx.fillStyle = '#8A9BB5'
          ctx.font = '500 22px Satoshi, Arial'
          ctx.fillText('📍 ' + loc, 50, nameY + 118)
        }
        ctx.fillStyle = '#8A9BB5'
        ctx.font = '500 18px Satoshi, Arial'
        ctx.fillText('Bitcoin Abuja · Powered by Fedi', 50, H - 44)
        const qrSize = 280
        ...
        ctx.lineWidth = 2
        rrect(ctx, qrX - 16, qrY - 16, qrSize + 32, qrSize + 32, 16)
        ctx.fill()
        ctx.stroke()
        ...
        ctx.fillText('Scan to pay', qrX + qrSize / 2, qrY + qrSize + 24)
        ctx.fillStyle = '#2DD4BF'
        ctx.font = '600 16px Satoshi, Arial'
        ctx.fillText('No POS needed', qrX + qrSize / 2, qrY + qrSize + 48)
      } else {
        ctx.fillStyle = '#FFFFFF'
        ctx.fillRect(0, 0, W, H)
        ...
        ctx.textBaseline = 'top'
        ctx.fillStyle = '#F97316'
        ctx.font = '800 44px Satoshi, Arial'
        ctx.fillText('BITCOIN ACCEPTED HERE', W / 2, 130)
        ctx.fillStyle = '#0B1220'
        ctx.font = '900 80px Satoshi, Arial'
        fitText(ctx, name, W / 2, 196, W - 120, 88, 2)
        ctx.fillStyle = '#2DD4BF'
        ctx.font = '700 34px Satoshi, Arial'
        ctx.fillText('⚡ Lightning payments. Instant settlement.', W / 2, 398)
        if (loc) {
          ctx.fillStyle = '#8A9BB5'
...
export default function App() {
  ...
  <div style={{ padding: '6px 14px 0', display: 'flex', flexDirection: 'column', gap: 10 }}>
    ...
    <div style={{ padding: '12px 2px 6px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
